...
//count nature[1]                   
$comp_query3 = "SELECT COUNT(complaint_ID) FROM complaint_table WHERE complaint_nature = 'Dirty Barangay' AND WEEK(complaint_date) = WEEK(now());";
$comp_result3 = $conn -> query($comp_query3);
$comp_row3 = $comp_result3 -> fetch_array();
$dirty = $comp_row3[0];

//count nature[2]                   
$comp_query4 = "SELECT COUNT(complaint_ID) FROM complaint_table WHERE complaint_nature = 'Drugs' AND WEEK(complaint_date) = WEEK(now());";
$comp_result4 = $conn -> query($comp_query4);
$comp_row4 = $comp_result4 -> fetch_array();
$drugs = $comp_row4[0];

//count nature[3]                   
$comp_query5 = "SELECT COUNT(complaint_ID) FROM complaint_table WHERE complaint_nature = 'Gossip Mongers' AND WEEK(complaint_date) = WEEK(now());";
$comp_result5 = $conn -> query($comp_query5);
$comp_row5 = $comp_result5 -> fetch_array();
$gossip = $comp_row5[0];

//count nature[4]                   
$comp_query6 = "SELECT COUNT(complaint_ID) FROM complaint_table WHERE complaint_nature = 'Noise' AND WEEK(complaint_date) = WEEK(now());";
$comp_result6 = $conn -> query($comp_query6);
$comp_row6 = $comp_result6 -> fetch_array();
$noise = $comp_row6[0];

//count nature[1]                   
$comp_query7 = "SELECT COUNT(complaint_ID) FROM complaint_table WHERE complaint_nature = 'Other' AND WEEK(complaint_date) = WEEK(now());";
...
$other = $comp_row7[0];
	...
	$html .="
			...
			$html .="
				...
				";
				$query9 = "SELECT DISTINCT complaint_nature FROM complaint_table WHERE week(complaint_date) = week(now());";
    ...
    $dompdf->stream("$week-WEEKLYCOMPLAINTREPORT.pdf", ["Attachment" => 0]);
?>