...
<?php
    if (isset($_POST['start'])){
        ...
        $allData = '';
        $resultm = $conn -> query("SELECT * FROM tblofficial LIMIT $start, 50;");
        while($row = $resultm->fetch_assoc()){
            $allData .= $row["official_id"] . ',' . $row["resident_id"] . ',' . $row["user_id"] . ',' . $row["name"] . ',' . $row["position"] . ',' . $row["term_start"] . ',' . $row["term_end"] . ',' . $row["status"] . ',' . "\n";
        }
        exit(json_encode(array("data" => $allData)));
    }
    $sql = $conn -> query("SELECT official_id FROM tblofficial;");
...
<body>
    <div class="main_container d-flex">
        ...
        ?>">
            ...
            <li class="active"><a href="../official_management/official_management.php" class="text-decoration-none px-3 py-2 d-block"><i class="fa-solid fa-image-portrait"></i>&nbsp;Official</a></li>
        ...
        <div class="content">
            ...
            <div class="dashboard-content px-3 py-4">
                ...
                <?php 
                if (isset($_SESSION["importOfficial"])){
                    if($_SESSION["importOfficial"] == "success"){?>
                ...
                <?php 
                    }else if($_SESSION["importOfficial"] == "fail"){?>
                ...
                <?php 
                if(isset($_SESSION['OffMessage']))
                {
                    if($_SESSION['OffMessage'] != ''){?>
                    <div class="alert alert-<?php if($_SESSION['sOffMessage'] == 1){ echo 'success';} else { echo 'danger';}?> mt-2" role="alert"><?php echo $_SESSION['OffMessage'];?></div> 
                <?php
                    $_SESSION['OffMessage'] = '';
                    }
                } ?>
    ...
    <!--End Modal-->
    <div class="modal fade" id="endModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
    ...
    <!-- End official term script-->
    <script>
        $(document).ready(function(){
            $('.endterm').click(function(){
                var offid = $(this).data('id');
                $.ajax({url: "end_form.php",
                method:'post',
                data: {offid:offid},
    ...
    <script>
        ...
        function exportToCSV(start, max){
            if (start > max){
                $("#response").html('<a href="'+data+'" download = "Officials Table" class="btn btn-outline-primary">Export</a>');
                return;
            }
            $.ajax({ url: "official_management.php",
            method: 'POST',
            dataType: 'json',
            data: {start: start}, 
